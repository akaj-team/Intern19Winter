...
class TodoListDatabaseHelper(context: Context) :
    ...
    companion object {
        ...
        private const val SELECT_TABLE_USER = "SELECT * FROM $TABLE_USER WHERE $KEY_USER_ID = ?"
        private const val SELECT_TABLE_USER_LOGIN = "SELECT * FROM $TABLE_USER WHERE $KEY_USER_NAME = ? AND $KEY_USER_PASSWORD = ?"
        private const val SELECT_TABLE_USER_ALL = "SELECT * FROM $TABLE_USER"
        private const val SELECT_TABLE_TODO = "SELECT * FROM $TABLE_TODO WHERE $KEY_USER_ID_PK = ?"
        private const val SELECT_TABLE_TODO_DONE = "SELECT * FROM $TABLE_TODO WHERE $KEY_USER_ID_PK = ? AND $KEY_TODO_STATUS = ?"
    }
    ...
    fun readUserLogin(userName: String, password: String): MutableList<User> {
        ...
        try {
            cursor = db.rawQuery(SELECT_TABLE_USER_LOGIN, arrayOf(userName, password))
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast) {
                ...
                users.add(User(id, username, pass, nickName, path))
                cursor.moveToNext()
            }
        }
        cursor.close()
    ...
    fun readUser(userId: Int): User {
        ...
        Log.d("xxx", "ID: $userId")
        try {
            cursor = db.rawQuery(SELECT_TABLE_USER, arrayOf(userId.toString()))
        } catch (e: SQLException) {
            db.execSQL(CREATE_TABLE_USER)
            return users[0]
        }
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast) {
                ...
                users.add(User(id, userName, password, nickName, path))
                cursor.moveToNext()
            }
        }
        Log.d("xxx", "User: ${users.size}")
        cursor.close()
        return users[0]
    }
